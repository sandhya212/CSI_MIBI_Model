...
IMM.MCMC <- function(r){
    ...
    #############################################################################################
    #############################################################################################
    full.data.subsample <- full.data.1[1:numcells,(1+(gene_batch*(r-1))):(gene_batch*r)];
    ...
    #####################################################################################################
        ...
        while(step <steps){ #begin of sampling loop
            ...
            pdf(file=f)
            plot(X_tsne$Y[,1],X_tsne$Y[,2],pch=16, col = col_palette[1*(as.numeric(factor(Class)))],  main=paste("t-SNE of X (true labels) step", step," batch ",r));
            dev.off();
        ...
        plot(X_tsne$Y[,1],X_tsne$Y[,2],pch=16, col = col_palette[1*(as.numeric(factor(Class)))],   main=paste("t-SNE of X (inferred labels) batch ",r));
        dev.off()
    ...
    return(list(z_inferred <- Class, mu <- mu, Sigma <- Sigma, sd_per_batch <- stddev.genes.batch))
...
for(r_all in 1:runs.all){
    ...
    results.per.MCMC <- foreach(r=((1+(num_gene_sub_batches*(r_all-1))):(num_gene_sub_batches*r_all)),.packages=c("Rtsne","lattice","MASS","bayesm","robustbase","chron","mnormt","MCMCpack", "coda","Matrix", "mvtnorm")) %dopar% {
        ...
        return(list(unlist(values[[1]]),values[[2]],values[[3]],values[[4]])) ##z, mu and Sigma
    }
